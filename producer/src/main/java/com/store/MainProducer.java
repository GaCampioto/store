    ...
    try (var orderDispatcher = new KafkaDispatcher<Order>()) {
      try (var emailDispatcher = new KafkaDispatcher<Email>()) {
        ...
        String userEmail = new Random().nextInt(999) + "@email.com";
          ...
          orderDispatcher.send(STORE_NEW_ORDER_TOPIC, userEmail, order, new CorrelationId(SERVICE_NAME));

          Email email = new Email(userEmail, "new order",
              "Your order " + orderNumber + " is being processed");
          emailDispatcher.send(SEND_EMAIL_TOPIC, userEmail, email, new CorrelationId(SERVICE_NAME));
        }
      }
    }

    try (var batchDispatcher = new KafkaDispatcher<String>()) {
      batchDispatcher.send(SEND_MESSAGE_TO_ALL_USER_TOPIC, REPORT_FOR_USER, REPORT_FOR_USER, new CorrelationId(SERVICE_NAME));
    }
  }
}
